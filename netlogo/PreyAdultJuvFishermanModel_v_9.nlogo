...
;  ]
  ...
  set clock clock + 1
  check-breeding-prey ;---------------checks breeding time for prey species / turns off breeding if capacity is hit
...
to grim-reaper-prey
  if clock mod 10 = 0 [
    if random-float 100 < 1 [ ;----------------------------------discrete-method natural mortality occurs at time step interval [everytime tick = 100]
...
to reproduce-prey
  if time-to-breed-prey = true [   ;;------------------------------will only breed at specified time interval between clock mod 80 = 0 and clock mod 100 = 0       
  ...
  ifelse any? prey in-radius 2
    [ let candidates one-of prey in-radius 2
      ...
      ask patch-ahead 1[
        if(pcolor = grey - 2)
        [
          set wall? true  ;;--------------------check if there are islands or a port between them
        ]
        if(pcolor = lime)
        [
          set wall? true
      ...
      ifelse (wall?)
      [
        rt random 360 ;;------------------------if there are either, random move
        move
      ]
