...
;; INITIAL MISC.
;;-----------------------------  
  set carrying-capacity  (count patches) / 4 ;;-----counts patches, possible later BehaviorSpace manipulation
  set sexy-time-adults 0
  set sexy-time-prey 0
  set clock 1
  set conversion-rate 0.5
end
...
to grim-reaper-prey ;--------------------------------------------mortality function contained with "grim-reaper" function
  if clock mod 10 = 0 [
    if random 100 <= 1 [ ;---------------------------------------discrete-method natural mortality occurs at time step interval [everytime tick = 100]. should match empirical estimates.
...
to reproduce-prey
  if time-to-breed-prey = true [   ;;-----------------------------will only breed if time-to-breed function returns "true" value     
    ...
    ifelse location != false [
      ...
      hatch random 100 [  ;;--------------------------------------prey is r-selective, so is much more fecund than predators. 
...
to cannibalize ;;------------------------------------------------sets up juvenile density-dependent cannibalism function
  ...
  ifelse yumyum >= 5 [
    if random 100 <= 49 [  ;-------------------------------------fit cannibalism rate to data from cod stomach analysis: also, going to be adjustable parameter for analysis
      let yummy count juveniles in-radius 1
      set cannibal-deaths cannibal-deaths + yummy
      set juvenile-total-deaths juvenile-total-deaths + yummy
      let moresnacks juveniles in-radius 1 
      let evenmoresnacks random count moresnacks
      ask n-of evenmoresnacks moresnacks [die]
      set metabolism metabolism + (0.2) * evenmoresnacks
    ]
  ]
  [
    swim-adults
...
to reproduce-adults
  if time-to-breed-adults and not done-breeding[
  let location false                  
  ask patch-here[
    if  pcolor = red               
    [ set location true
    ...
    ifelse location != false [
      hatch-juveniles round(conversion-rate * metabolism)  [  ;;-------------------------adult turtle breed gives birth to juvenile turtle breed.
...
to grim-reaper-juveniles ;-------------------------------------------------------juvenile mortality functions
  if clock mod 10 = 0 [
    if random-float 100 < 1 [ ;--------------------------------------------------natural mortality rate utilizing empirical parameters
...
to fish-adults                       
  if random 100 <= 90                  ;;------------------------------------------modelled on trawling. final model will contain adjustable fishing-success rate.
    [
        if any? adults-here [
...
277
NIL
count adults with [done-breeding]
17
1
11
