...
to define-parameters
  ...
  set initial-number-prey 50
  set initial-number-adults 50
  set initial-number-juveniles 50
  set initial-number-fishermen 5
  set adult-breeding-ground 1
  set prey-breeding-ground 1
  set breeding-ground-area 5
  set number-of-ports 1
  set cannibal-deaths 0
  set cannibal-probability 0
  set clock 0
...
to config-ocean
  ...
  [
    let q self
    let b max list 1 round(random-normal breeding-ground-area ( breeding-ground-area *  sd-island-area ))
  ...
  [
    let r self
    let c max list 1 round(random-normal breeding-ground-area ( breeding-ground-area *  sd-island-area ))
  ...
  [
    let s self
    let d max list 1 round (random-normal 2(1.5))
...
to setup-prey
  create-prey initial-number-prey [
    ...
    let x 0
    let y 0
    ask one-of patches with [pcolor != grey - 2]
    [ 
      set x pxcor
      set y pycor
    ]
...
to go
  ...
  ask prey [
    ;grim-reaper-prey ;------------------removed to match discrete model
    reproduce-prey
  ]
  ...
  ask juveniles [
    set age age + 1
    swim-juveniles]
  repeat 5 [ask juveniles [fd 0.1] display ]
  ask juveniles [
  ...
  ask fishermen [
    ;remember ;--------- this will be how we make fishermen remember the best fishing spots
    boat-move
    set fuel fuel - 1]
  repeat 5 [ask fishermen [fd 0.2] display ]
  ask fishermen[
    if Adults-Only? [
      ask fishermen [
  ...
  set clock clock + 1
  set cannibal-probability ( cannibal-deaths / clock )
  tick
end
...
;;-----------------------------
;; CAPELIN REPRODUCE FUNCTION
;;-----------------------------


to reproduce-prey
  let time-to-breed false
  if clock mod 100 = 0 [
    set time-to-breed true ]
  if time-to-breed = false [
    swim-prey]
  ifelse time-to-breed [
  ...
  ask patch-here[
    ...
    [swim-prey] ;-- end of locality command
  ; ; -- end of time-to-breed command
  swim-prey
  trace-back 
...
to grim-reaper-adults
  if clock mod 10 = 0 [
    if random-float 100 < 1 [
      die
      set natural-deaths-adults natural-deaths-adults + 1 
  ]  
  ]
  if age >= 1400 [
    die
    set natural-deaths-adults natural-deaths-adults + 1]      ;;-----------------------age-bounded mortality
end
...
to reproduce-adults
  ...
  ask patch-here[
    ...
    [swim-adults] ;-- end of locality command
  ; ; -- end of time-to-breed command
  swim-adults
  trace-back-adults
  set time-to-breed false      
...
to fish-adults                       
  if random 100 <= 49                  ;;----------adjustable fishing successs rate
    [
        if any? adults-here [
          let target one-of adults
          ask target [
            die
          ]
          set fishing-deaths-adults fishing-deaths-adults + 1
          set total-catch total-catch + 1
          set catch catch + 1
          set adults-caught adults-caught + 1
        ]
    ]
    boat-move
end 
  ...
  if random 100 <= 49                  ;;----------adjustable fishing successs rate
    [
        if any? prey-here [
          ...
          set fishing-deaths-prey fishing-deaths-prey + 1
          set total-catch total-catch + 1
          set catch catch + 1
          set prey-caught prey-caught + 1
        ]
    ]
    boat-move
end 
  ...
  if random 100 <= 49                  ;;----------adjustable fishing successs rate
    [
        if any? juveniles-here [
          let j-target one-of juveniles
          ask j-target [
            die
          ]
          set fishing-deaths-juveniles fishing-deaths-juveniles + 1
          set total-catch total-catch + 1
          set catch catch + 1
          set juveniles-caught juveniles-caught + 1
        ]
    ]
    boat-move
end
...
;;-----------------------------
;; FISHERMEN BEHAVIORAL FUNCTIONS
;;-----------------------------

  
to check-catch-limit
  if catch >= 50 [
    dropoff
  ]
  if Adults-Only? [
