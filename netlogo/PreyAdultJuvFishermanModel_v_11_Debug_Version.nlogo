...
to grim-reaper-prey ;--------------------------------------------mortality function contained with "grim-reaper" function
  if clock mod 10 = 0 [
    if random-float 100 < 1 [ ;----------------------------------discrete-method natural mortality occurs at time step interval [everytime tick = 100]. should match empirical estimates.
...
to reproduce-prey
  if time-to-breed-prey = true [   ;;-----------------------------will only breed if time-to-breed function returns "true" value     
    ...
    ifelse location != false [
      if count(prey) <= carrying-capacity [ ;;--------------------imposes density dependence. carrying capacity is set to area of IBM (50x50 = 2500)
...
to hunt ;;-------------------------------------------------------tells adults to eat any prey within radius 1 at some rate of successs
  ifelse any? prey in-radius 1 [
    if random 100 <= 59 [
      ...
      set metabolism metabolism + yummy ;;------------------------going to be used for resource-dependent reproductive success
    ]
  ]
  [
  school-adults
...
to cannibalize ;;------------------------------------------------sets up juvenile density-dependent cannibalism function
  ...
  ifelse yumyum >= 5 [
    if random 100 <= 49 [  ;-------------------------------------fit cannibalism rate to data from cod stomach analysis: also, going to be adjustable parameter for analysis
      ...
      set metabolism metabolism + yummy
    ]
  ]
  [
    swim-adults
...
to reproduce-adults
  ...
  ask patch-here[
    ...
    ifelse location != false [
      ...
      set metabolism 0 ; ---------------------------------------------------------------------------------end of hatch command
      ]
    [trace-back-adults]
    ]
    swim-adults
