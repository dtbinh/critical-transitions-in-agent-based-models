...
;; INITIAL SEEDING
;;-----------------------------
  set initial-number-prey initial-number-prey   ;;---------------------BehaviorSpace Parameter
  set initial-number-adults initial-number-adults ;;-------------------BehaviorSpace Parameter
  set initial-number-juveniles initial-number-juveniles ;;-------------BehaviorSpace Parameter
...
;;-----------------------------  
  ...
  set clock 1
  set conversion-rate conversion-rate
...
to grim-reaper-prey ;--------------------------------------------mortality function contained with "grim-reaper" function
  if clock mod 10 = 0 [
    if random 100 + 1 < (( prey-mortality-rate * 100 ) / 10) [ ;---------------------------------------discrete-method natural mortality occurs at time step interval [everytime tick = 100]. should match empirical estimates.
...
to reproduce-prey
  if time-to-breed-prey = true [   ;;-----------------------------will only breed if time-to-breed function returns "true" value     
    ...
    ifelse location != false [
      ...
      hatch intrinsic-growth-rate * 100 [  ;;----prey is r-selective, so is much more fecund than predators. 
...
to grim-reaper-adults ;;---------------------------------------------------------------adult mortality functions.
  if clock mod 10 = 0 [
    if random 100 + 1 < (( adult-mortality-rate * 100 ) / 10) [
...
to grim-reaper-juveniles ;-------------------------------------------------------juvenile mortality functions
  if clock mod 10 = 0 [
    if random 100 + 1 < (( juvenile-mortality-rate * 100 ) / 10) [ ;--------------------------------------------------natural mortality rate utilizing empirical parameters
...
405
214
Biological Parameters (Juvenile)
11
0.0
1
